...
### Fixed
- **🔧 ESC Key with Popovers**: First ESC now only closes the popover (tags, mentions, properties), second ESC changes mode
- **🌐 WebView Watchdog**: Auto-reload mechanism if WebView doesn't complete loading within 300ms
- **💬 Chat AI Navigation**: Fixed UI freeze when loading notes from Chat AI links
- **⌨️ Cursor Position in INSERT Mode**: Fixed Enter/Backspace/Tab cursor jumping when cursor was moved with mouse

### Technical
- Added `webview_load_watchdog` and `webview_load_completed` fields for WebView state tracking
- Cursor position now syncs from GTK before operations in INSERT mode
- State reset when transitioning from Chat AI to note view
